...
parseEmacsLisp str codeLang = 
  let tofinde = "```" ++ codeLang
      ...
      getCode = LU.takeWhileList (\xs -> (take 3 xs) /= "```") dropPrefix
  ...
  if null emacsCode
    then return ""
            ...
            let today = formatTime defaultTimeLocale "%d.%m.%y" now
                headline = ";; " ++ today ++ " Generated by markdown2el @ 2017 \n\n\n"
                ctx = headline ++ emacsCode
...
processFiles fromPath toPath fileExt codeLang = do
  markdownFiles <- case fileExt of 
    ...
    _ -> return ([])
  putStrLn "Find markdown files: " >> mapM_ print markdownFiles
